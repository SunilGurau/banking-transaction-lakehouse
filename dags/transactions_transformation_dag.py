...
DBT_VARS_TEMPLATE = '{{ {"batch_table_uris": {"transactions": ti.xcom_pull(task_ids="resolve_transaction_file")}} | tojson }}'
...
def transactions_transformation():
    ...
    dbt_run_stage = BashOperator(
        ...
        bash_command=(
            ...
            f"--target {DBT_TARGET} "
            f"--select stg_batch__transactions "
            f"--vars '{DBT_VARS_TEMPLATE}'"
        ),
    )

    transaction_file = resolve_transaction_file()
    transaction_file >> dbt_run_stage
